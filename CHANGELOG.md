...
- **Per-slot controller names (fleet bookkeeping for many near-identical pads).** Each pairing slot can carry a 12-char user label ("Volcanic Red") stored in the slots flash sector (layout v2, automatic one-time migration preserving existing bonds; a slot's name is cleared when a *different* controller takes the slot). Shown on both Slots screens (label wins over the hex address) and served over USB: the `0xFA` feature report grows a names section (76 bytes total, reqlen-gated so legacy 28-byte readers are untouched), and `0xF6` gains hardened func `0x11` (`'S''L'` + version gate) with ops set-addr+name / set-name / clear. `set_ds5.py`: `--slots` dumps the table (addr + name — build your controller database from this), `--slot-set N --slot-addr AA:BB:CC:DD:EE:FF --slot-name "..."` provisions or renames, `--slot-clear N` wipes. Note: provisioning an address restores slot *routing*; the BT link key lives in BTstack NVM, so a pad never bonded to that dongle still needs one Create+PS pairing pass.
- **8-Player extension: `player_id` now spans 1..8** (was 1..4) across firmware, both display variants and the CLI. P5–P8 get fork-defined identities — orange / cyan / purple / yellow accents on the LCD, matching lightbar stamp colours, and distinct player-LED patterns on the DualSense's 5-LED bar (Sony defines only P1–P4; the extra masks are ours). Selecting P5–P8 **defaults BT mic off** for that dongle (2.4 GHz airtime is the scaling constraint; audio streams are the hog) — still overridable via the Settings mic row or `--bt-mic on`. `set_ds5.py` gains `--player 5..8` and an explicit `--bt-mic {on,off}` flag. Scaling reminder: it's still one controller per dongle — going past four is about buying dongles, spreading the radios apart, and a powered (ideally multi-TT) USB 2.0 hub.
...
- **"Pico LED off" now applies the moment you save** (both display variants and the 0xF6 web-config path). Root cause: the Settings screens save through `set_config(const Config_body&)`, which — unlike the web-config overload — never touched the LED, so the toggle only took effect on the next disconnect/reconnect edge. Both overloads now call a single `pico_led_apply()` policy point (LED on ⇔ connected AND not disabled); this also fixes the web-config path lighting the LED while no controller was connected. The critical-battery blink still deliberately overrides the preference. (`src/config.{h,cpp}`)
- **Clip-safe display strings on both panels.** Six strings were authored wider than the writable area (LCD: 18 chars at scale 2 given the reserved A/B chrome column; OLED: 20 chars at 128 px) and ran off the right edge — most visibly the pairing screen's "2. Wait for light bar". Re-wrapped/shortened, with the per-panel width budget now documented at the fix sites. (`src/oled.cpp`, `src/lcd.cpp`)

### Changed

- **Latency screen readouts are now in milliseconds** on both variants: "Lat avg / max / pk" (dongle transit, formerly the µs "Trans/Peak" rows), BT gap, and the display-busy budget all render as `x.xx ms` via a shared clamped formatter (`fmt_ms`, caps at `99.9+` to hold the row-width budget). Raw µs fields in the `0xFD` vendor report are unchanged — display formatting only.
